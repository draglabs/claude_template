...
| "you are the Developer" | Developer (Default) | [`docs/dev_framework/developer.md`](docs/dev_framework/developer.md) + [`docs/dev_framework/coding-standards.md`](docs/dev_framework/coding-standards.md) + [`dev_framework_exceptions.md`](docs/framework_exceptions/dev_framework_exceptions.md) + active plan's `plan.md`. Hands-on coding in your main checkout on a feature branch (`w-<id>/<slug>`). Bootstrap proposes top critical-path `pending` item. Rewind ritual + blind self-review ([ADR-018](docs/architecture/adr-018-developer-role.md)). Mixed-mode phases allowed. |
| "you are the parallel developer" | Developer (Parallel) | Same role doc + Layer 1 reads as Default. Works in a **worktree** at `/tmp/worktrees/<project>/w-<id>-<slug>` instead of the main checkout. Bootstrap does the **non-competing scan** (avoids items that overlap with already-claimed work). Use when running alongside the Default Developer for coding-throughput parallelism. Same rewind ritual, same lifecycle. |
