...
**Why code-level docs are not in Layer 1.** Code-quality rules (TDD, no hardcoded values, fail loudly) are enforced at the subagent layer, not by the Orchestrator or Strategist. Keeping `coding-standards.md` out of the Orchestrator's and Strategist's Layer 1 preserves their context for decision-making and plan-keeping. The Executor loads the doc to write correctly; the Reviewer loads it to enforce.
